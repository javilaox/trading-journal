...
<style>
  ...
  *{box-sizing:border-box}
  ...
  .count-btn{min-width:38px;padding:7px 12px;border-radius:10px;cursor:pointer;font-family:inherit;
  ...
  .count-btn.active{background:rgba(56,189,248,.16);border-color:rgba(56,189,248,.6);color:#bae6fd;font-weight:700}
...
(function () {
  ...
  /* "Y si compro varios challenges": apartado propio con su selector, en lenguaje llano.
     El estado vive en CHALLENGE_CTX porque el bloque se redibuja solo al cambiar el numero. */
  var CHALLENGE_CTX = null;

  function pickChallengeCount(n) {
    if (!CHALLENGE_CTX) return;
    CHALLENGE_CTX.picked = Math.max(1, Math.min(6, Number(n) || 1));
    renderRotationBlock();
  }
  ...
  function renderRotationBlock() {
    ...
    var rows = compareChallengeAccounts(ctx.rs, ctx.cfg, { runs: 1200, tradesPerDay: ctx.perDay }, 6);
    if (!rows.length) { host.innerHTML = ''; return; }

    var current = null;
    for (var i = 0; i < rows.length; i += 1) if (rows[i].accounts === picked) current = rows[i];
    ...
    host.innerHTML =
      ...
      '<p class="muted small">Elige cuántos y mira cómo cambia. Se opera una cuenta cada vez, así que ' +
      'el ritmo diario es el mismo: lo único que cambia es cuándo se salta de una cuenta a otra.</p>' +
      '<div class="count-picker"><span class="muted small">Compro</span>' +
      rows.map(function (r) {
        return '<button type="button" class="count-btn' + (r.accounts === picked ? ' active' : '') +
          '" data-count="' + r.accounts + '">' + r.accounts + '</button>';
      }).join('') +
      '<span class="muted small">' + (picked === 1 ? 'challenge' : 'challenges') + '</span></div>' +
      (picked === 1
        ? '<p class="muted small">Con un solo challenge no hay nada que rotar: la probabilidad de pasarlo es ' +
          pct(rows[0].rotating.anyPassRate) + '. Pulsa 2 o más para comparar las dos formas de gestionarlos.</p>'
        : '<div class="modes">' +
            modeCard('Voy cambiando de cuenta',
            ...
            modeCard('Exprimo una y luego la siguiente',
          ...
          '<p class="muted small">' +
          (rotWins
            ? 'Con ' + picked + ' cuentas compensa ir cambiando: se pasan ' + current.rotating.avgAccountsPassed.toFixed(2) +
              ' de media frente a ' + current.sequential.avgAccountsPassed.toFixed(2) + '.'
            : seqWins
              ? 'Con ' + picked + ' cuentas sale mejor exprimir una antes de abrir la siguiente: ' +
                current.sequential.avgAccountsPassed.toFixed(2) + ' de media frente a ' + current.rotating.avgAccountsPassed.toFixed(2) + '.'
      ...
      '<details class="rot-details"><summary>Ver la comparativa de 1 a 6 challenges</summary>' +
      '<div class="table-scroll"><table><thead><tr>' +
      '<th class="no-sort">Compro</th>' +
      '<th class="num no-sort">Cambiando · pasas ≥1</th><th class="num no-sort">Cambiando · media</th>' +
      '<th class="num no-sort">Una a una · pasas ≥1</th><th class="num no-sort">Una a una · media</th>' +
      '</tr></thead><tbody>' +
      rows.map(function (r) {
  ...
  document.addEventListener('click', function (e) {
    var btn = e.target.closest ? e.target.closest('[data-count]') : null;
    if (btn) pickChallengeCount(btn.getAttribute('data-count'));
  });
