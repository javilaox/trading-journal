...
<div class="wrap hidden" id="app">
  ...
  <section id="viewList" class="hidden">
    <div class="card">
      ...
      <div id="tradeList"><p class="muted small">Cargando…</p></div>
    </div>
  </section>
...
(function () {
  ...
  async function startApp() {
    ...
    resetForm();
    await Promise.all([loadCatalogs(), loadTrades()]);
  }

  /* ─────────────────────── Cuentas y estrategias ─────────────────────── */

  async function loadCatalogs() {
    var accRes = await db.from('real_accounts').select('id,name').order('name');
    ...
    fillSelect($('fAccount'), ACCOUNTS.map(function (a) { return a.name; }), localStorage.getItem('lastAccount'));
  ...
  ['segDirection', 'segResult', 'segBeAfter', 'segPeriod'].forEach(function (id) {
  ...
  function renderList() {
    var host = $('tradeList');
    if (!TRADES.length) {
      host.innerHTML = '<p class="muted small">Aún no hay trades.</p>';
      return;
    }
    host.innerHTML = TRADES.slice(0, 60).map(function (t) {
      ...
      return '<div class="trade" data-id="' + t.id + '">' +
        '<div class="trade-main">' +
          ...
          '<small>' + displayDate(t.date) + (t.strategy ? ' · ' + escapeHtml(t.strategy) : '') +
          (t.direction ? ' · ' + (t.direction === 'SHORT' ? 'Venta' : 'Compra') : '') + '</small>' +
        '</div>' +
  ...
  async function editTrade(id) {
    var t = TRADES.filter(function (x) { return Number(x.id) === id; })[0];
    if (!t) return;
    resetForm();
    EDITING = t;
    netTouched = true;
  ...
  function renderStats() {
    var list = TRADES.filter(periodFilter());
  ...
  function showView(view) {
    ...
    $('viewTitle').textContent =
      view === 'list' ? 'Trades' : view === 'stats' ? 'Resumen' : (EDITING ? 'Editar trade' : 'Nuevo trade');
  ...
  $('nav').addEventListener('click', function (e) {
    ...
    if (view === 'list' || view === 'stats') loadTrades();
    showView(view);
  });
