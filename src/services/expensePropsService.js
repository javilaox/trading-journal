/**
 * Listas de nombres reutilizables de Gestión (props y categorías de gasto) — offline-first
 * (SQLite + sync_queue + Supabase).
 *
 * Las dos son exactamente la misma entidad: un nombre por usuario, sin duplicados, que
 * sobrevive a los movimientos que lo usan. Por eso está escrito una sola vez y se instancia por
 * tabla, en vez de tener dos copias que se vayan separando con el tiempo.
 *
 * Importante: estas listas NO tocan los movimientos. En gastos y retiros, la prop y la
 * categoría se guardan como texto en la propia fila, así que borrar un nombre de la lista deja
 * intactos los movimientos que ya lo usaban.
 */

function createNameListService(table) {
  ...
  function upsertPropsIntoLocal(db, remoteRows, userId, logPrefix = '') {
    ...
    const selectByClient = db.prepare(
      `SELECT id, sync_status, deleted_at FROM ${table} WHERE user_id = ? AND client_uuid = ? LIMIT 1`
    );
    const selectByName = db.prepare(
      `SELECT id, sync_status, deleted_at FROM ${table} WHERE user_id = ? AND name = ? COLLATE NOCASE LIMIT 1`
    );
  ...
  return {
    isRowHidden: isPropRowHidden,
    mapRowToResponse: mapPropRowToResponse,
    getFromLocal: getPropsFromLocal,
    supabaseRowFromPayload,
    upsertIntoLocal: upsertPropsIntoLocal,
  };
}

const propsService = createNameListService('expense_props');
const categoriesService = createNameListService('expense_categories');

module.exports = {
  createNameListService,
  propsService,
  categoriesService,
  // Nombres originales, para no tocar lo que ya los importaba.
  isPropRowHidden: propsService.isRowHidden,
  mapPropRowToResponse: propsService.mapRowToResponse,
  getPropsFromLocal: propsService.getFromLocal,
  supabaseRowFromPayload: propsService.supabaseRowFromPayload,
  upsertPropsIntoLocal: propsService.upsertIntoLocal,
};
